...
var Controller = {
	...
	decodeHTML(encodedString) {
		...
		for (var i in this.entity_table) {
			if (i != 38) {
				encodedString = encodedString.replace(new RegExp(this.entity_table[i], "g"), String.fromCharCode(i));
			}
		}

		encodedString = encodedString.replace(new RegExp("&#(x?)(\\d+);", "g"), function(match, p1, p2, string) {
			return String.fromCharCode(((p1 == 'x') ? parseInt(p2, 16) : p2));
		});
		encodedString = encodedString.replace(new RegExp(this.entity_table[38], "g"), String.fromCharCode(38));
